...
const Index = () => {
  ...
  // Handle customer selection - UPDATED to always fetch fresh balance from database
  const handleCustomerSelect = (customerName: string) => {
    ...
    // Always get the most up-to-date balance from bills (not cached customer balance)
    if (customerPhone) {
...
Use "Confirm Bill" to save this bill.
  ...
  // Get customer balance and history for display - always fetch fresh from database
  const getCustomerBalance = (customerName: string) => {
    const customer = customers.find(c => c.name === customerName);
    if (customer?.phone) {
      // Use latest balance from bills to ensure accuracy
                    ...
                    {filteredCustomers.map((customer, index) => (
                      ...
                      >
                        <div className="flex justify-between items-center">
                          ...
                          {getCustomerBalance(customer.name) > 0 && (
                            <span className="text-red-600 text-xs font-medium">
                              Balance: ₹{getCustomerBalance(customer.name).toFixed(2)}
                            </span>
                          )}
                        </div>
                      </div>
